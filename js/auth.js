$(document).ready(function(){
	var url="https://www.sistemasycontroles.com/app_reportes_demo/api/sistema_2.0_api/auth.php?callback=?";
  ...
  $("#visitantes").click(function(){
    	...
    	var dataString="cedula="+cedula+"&nombre_completo="+nombre_completo+"&telefono="+telefono+"&empresa="+empresa+"&tipo_de_visita="+tipo_de_visita+"&contacto_unitec="+contacto_unitec+"&temperatura="+temperatura+"&eps="+eps+"&arl="+arl+"&observaciones="+observaciones+"&visitante=";
  ...
  $("#empleados").click(function(){
		...
		var nombre_completo=$("#contacto_unitec").val();
		var temperatura=$("#temperatura").val();
		var observaciones=$("#observaciones").val();

    	var dataString="nombre_completo="+nombre_completo+"&temperatura="+temperatura+"&observaciones="+observaciones+"&empleado=";

    	if($.trim(nombre_completo).length>0 & $.trim(temperatura).length>0)
		{
			$.ajax({
				type: "POST",
				url: url,
				...
				crossDomain: true,
				cache: false,
				beforeSend: function(){ $("#empleados").val('Conectando...');},
				success: function(data){
					if(data=="success")
					{
						alert("Se ha enviado tu registro al servidor");
